...
class BSECoopBrowserWindow(bui.Window):
    ...
    def _refresh(self) -> None:
        ...
        self._subcontainerheight = (
            620 + (90 if uiscale is bui.UIScale.SMALL else 0)
        )
        ...
        def pick_tip() -> dict:
            ...
            if tip[-4:] == ':WIN': #:WIN windows check
                if not bui.app.platform == 'windows': tip = pick_tip()
                else: tip = tip[:-4]

            return {
                'prefix': bui.Lstr(resource=f'explodinary.bsebrowser.{prefix}.header'),
                'text': bui.Lstr(translate=(f'bsetiptrans.{prefix}', tip[4:])),
                'chaos': is_chaos,
            }
        
        tip = pick_tip()
        txt = bui.Lstr(value='${A}\n${B}', subs=[
            ('${A}', tip['prefix']),
            ('${B}', tip['text']),
        ])
        ...
        if self._back_button is not None:
            bui.buttonwidget(edit=self._back_button, on_activate_call=self._back)
        else:
            bui.containerwidget(
