...
class ExplodinarySettings(bui.Window):
    ...
    def __init__(
        ...
        npos = ((width/8), (height*0.1 + (50 if uiscale is bui.UIScale.SMALL else 0)))
        wlab = "<"
        for x in range(2):
            ...
            npos = (width-(width/8)-30, (height*0.1 + (50 if uiscale is bui.UIScale.SMALL else 0)))
            wlab = ">"
    ...
    def refresh(self) -> None:
        ...
        if self._section == 0:
            ...
            bui.app.config['BSE: Powerup Distribution'] = bui.app.config.get('BSE: Powerup Distribution','Explodinary')
            if all_powerup_dists().get(bui.app.config['BSE: Powerup Distribution']) == None: # This should only happen when having an unkown dist.
            ...
            self._update_powerup_container(bui.app.config['BSE: Powerup Distribution'])
            ...
            cts = self._build_setting(
                ...
                choices_display=[bui.Lstr(resource=f'{thk}.y'),
            ...
            mt = self._build_setting(
                ...
                key='BSE: Menu Theme',
                default='Classic',
                choices=['Classic','Adventure','Island','Forest','Mountain', 'Spooky'],
                choices_display=[bui.Lstr(resource=f'{thk}.00'),
