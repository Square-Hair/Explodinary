...
def wavedash(self) -> None:
    if not bs.app.config.get('BSE: Quickturn',True): return # Cheeky
    if not self.node:
        return
    ...
    if t_ms - self.last_wavedash_time_ms >= self._wavedash_cooldown:
        ...
        bs.emitfx(position=self.node.position,
                  ...
                  chunk_type='spark',
                  count=10 if not bs.app.config.get("BSE: Reduced Particles", False) else 5,
...
def new_factory(func):
    def wrapper(*args, **kwargs):
        ...
        conditions=('they_have_material', bascenev1lib.gameutils.SharedObjects.get().footing_material),
        actions=(('message', 'our_node', 'at_connect', FootConnectMessage),
                 ('message', 'our_node', 'at_disconnect', FootDisconnectMessage)))
    return wrapper
bascenev1lib.actor.spazfactory.SpazFactory.__init__ = new_factory(bascenev1lib.actor.spazfactory.SpazFactory.__init__)

def new_handlemessage(func):
    def wrapper(*args, **kwargs):
        if args[1] == FootConnectMessage:
            args[0].grounded += 1
        elif args[1] == FootDisconnectMessage:
