...
class Particle(bs.Actor):
    ...
    def _fade_out(self):
        if self.node:
            ...
            bs.timer(
                self.attrs["fade"], bs.Call(self.handlemessage, bs.DieMessage())
            )
...
class bseVFX:
    def __init__(
        ...
        if mode == "vitalbomb":
            ...
            # Sparks
            sparkpos = [
                (p if i != 1 else p + 0.45) for i, p in enumerate(position)
            ]
            ...
            self._generate(
                "particleLeaf",
                "theBeginningLevelColor",
                position,
                (
                    [
                        (
                            v * 0.1
                            + (random.randint(-1, 1) * random.uniform(0.1, 3))
                        )
                        for v in velocity
                    ]
                ),
            ...
            if not hq and random.random() > 0.6:
                return
            ...
            # If low quality, give us a chance to *not* spawn snowflakes
            if not hq and random.random() > 0.8:
                return
            ...
            itr = 24 if hq else 8
            texlist = [
                f"confetti{v}" for v in ["Blue", "Green", "Red", "Yellow"]
            ]
            self._generate(
                "particleConfetti",
                texlist,
