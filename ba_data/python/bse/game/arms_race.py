...
# Slightly modified for BSE by SoK & Translated to api8 by Temp
# epic.
...
class State:
    ...
    def apply(self, player, spaz):
        ...
        release_input = (bs.InputType.PUNCH_RELEASE, bs.InputType.PICK_UP_RELEASE)
...
states = [State(bomb='normal',      raw_name = 'Normal Bombs' , name = bs.Lstr(translate=['armsRace', 'Normal Bombs' ])),
          State(bomb='ice',         raw_name = 'Ice Bombs'    , name = bs.Lstr(translate=['armsRace', 'Ice Bombs'    ])),
          State(bomb='sticky',      raw_name = 'Sticky Bombs' , name = bs.Lstr(translate=['armsRace', 'Sticky Bombs' ])),
          State(bomb='impact',      raw_name = 'Impact Bombs' , name = bs.Lstr(translate=['armsRace', 'Impact Bombs' ])),
          State(bomb='tacky',       raw_name = 'Tacky Bombs'  , name = bs.Lstr(translate=['armsRace', 'Tacky Bombs'  ])),
          State(bomb='toxic',       raw_name = 'Toxic Bombs'  , name = bs.Lstr(translate=['armsRace', 'Toxic Bombs'  ])),
          State(bomb='steampunk',   raw_name = 'Steam Bombs'  , name = bs.Lstr(translate=['armsRace', 'Steam Bombs'  ])),
          State(bomb='cluster',     raw_name = 'Cluster Bombs', name = bs.Lstr(translate=['armsRace', 'Cluster Bombs'])),
          State(grab=True,          raw_name = 'Grabbing only', name = bs.Lstr(translate=['armsRace', 'Grabbing only'])),
          State(punch=True,         raw_name = 'Punching only', name = bs.Lstr(translate=['armsRace', 'Punching only'])),
          State(curse=True,         raw_name = 'Cursed'       , name = bs.Lstr(translate=['armsRace', 'Cursed'       ]), final=True)]
