from __future__ import annotations

import ba

from typing import TYPE_CHECKING
if TYPE_CHECKING:
    from typing import Any, Sequence, Callable

from bastd.actor.spawner import Spawner

class BSESpawner(Spawner):
    def __init__(self,
                 data: Any = None,
                 pt: Sequence[float] = ...,
                 spawn_time: float = 1,
                 send_spawn_message: bool = True,
                 spawn_callback: Callable[[], Any] | None = None):
        super().__init__(data, pt, spawn_time, send_spawn_message, spawn_callback)
        self._light.color = (0.2, 0.4, 1)

        scl = float(spawn_time) / 3.75
        min_val = 0.4
        max_val = 0.95

        ba.animate(
            self._light,
            'intensity',
            {
                0.0: 0.0,
                0.25 *  scl: max_val,
                0.500 * scl: min_val,
                0.750 * scl: max_val,
                1.000 * scl: min_val,
                1.250 * scl: 1.1 * max_val,
                1.500 * scl: min_val,
                1.750 * scl: 1.2 * max_val,
                2.000 * scl: min_val,
                2.250 * scl: 1.3 * max_val,
                2.500 * scl: min_val,
                2.750 * scl: 1.4 * max_val,
                3.000 * scl: min_val,
                3.250 * scl: 1.5 * max_val,
                3.500 * scl: min_val,
                3.750 * scl: 2.0,
                4.000 * scl: 0.0,
            },
        )