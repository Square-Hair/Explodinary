...
class AllSettingsWindow(ba.Window):
    ...
    def __init__(
        ...
        width = 900 if uiscale is ba.UIScale.SMALL else 750
        ...
        if ba.app.ui.use_toolbars and uiscale is ba.UIScale.SMALL:
            ...
            self._back_button = btn = ba.buttonwidget(
                ...
                autoselect=True,
                position=(40 + x_inset + (45 if not uiscale is ba.UIScale.SMALL else 0), height - 55),
                size=(130, 60),
                scale=0.8,
                text_scale=1.2,
        ...
        basew = 200 if uiscale is ba.UIScale.SMALL else 217
        baseh = 170
        x_offs = (
            x_inset + (95 if uiscale is ba.UIScale.SMALL else 68) - basew
        )  # now unused
        ...
        ex_b_offs = x_inset + (20 if not ba.UIScale.SMALL else 0)

        def _b_title(
            x: float, y: float, button: ba.Widget, text: str | ba.Lstr
        ):
            return(ba.textwidget(
                parent=self._root_widget,
                text=text,
        ...
        exst = self._explodinary_settings_button = ba.buttonwidget(
            parent=self._root_widget,
            autoselect=True,
            position=(ex_offs, v),
            size=(basew, baseh),
        ...
        imgw = imgh = 120
        exsti = ba.imagewidget(
            parent=self._root_widget,
            position=(ex_offs + basew * 0.49 - imgw * 0.5 + 5, v + 35),
            size=(imgw, imgh),
            color=(0.8, 0.95, 1),
            texture=ba.gettexture('explodinarySettingsIcon'),
            draw_controller=exst,
        )
        exstt = _b_title(ex_offs, v, exst, ba.Lstr(resource='explodinary.bseSettingsWindow.titleShort'))

        v -= baseh - 5

        # Plugin manager offset
        try:
            from bastd.ui.settings.allsettings import AllSettingsWindow as aswold
        ...
        try:
            from bastd.ui.settings.allsettings import AllSettingsWindow as aswold
            aswold._do_modmanager
            exst = self._modmgr_button = self._plugin_manager_button = ba.buttonwidget(
                parent=self._root_widget,
                autoselect=True,
                position=(ex_offs, v),
                size=(basew, baseh),
                ...
                button_type='square',
                label='',
                on_activate_call=ba.Call(aswold._do_modmanager, self),
            )
            imgw = imgh = 120
            exsti = ba.imagewidget(
                parent=self._root_widget,
                ...
                size=(imgw, imgh),
                color=(0.8, 0.95, 1),
                texture=ba.gettexture('pluginsIcon'),
                draw_controller=exst,
            )
            ...
            #     text=ba.Lstr(resource=self._r + '.ex_settings'),
            # )
        self._restore_state()
    ...
    def _do_explodinary_settings(self) -> None:
        # pylint: disable=cyclic-import
        from explodinary.ui.settings.bsesettings import ExplodinarySettings
