...
class Map(Actor):
    ...
    def __init__(
        ...
        self.jump_points = self.get_def_points('jump') or []
        self.slingshot_points = self.get_def_points('slingshot') or []

        # We just want points.
        self.tnt_points = [p[:3] for p in self.tnt_points]
        self.jump_points = [p[:3] for p in self.jump_points]
        self.slingshot_points = [p[:3] for p in self.slingshot_points]
