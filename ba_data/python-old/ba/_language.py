...
class LanguageSubsystem:
    ...
    def translate(
        ...
        """
        try:
            # WHY IS THE TRANSLATIONS PATH HARDCODED
            # EFRO WHY IS THE CODE LIKE THIS
            # ALSO STOP RELEASING HOTFIXESSSSSSSSSS -T
            if category[:11] == 'explodinary':
                translated = self.get_resource('explodinary')[category[12:]][strval]
                
            elif category[:11] == 'bsetiptrans':
                translated = self.get_resource('explodinary')['bsebrowser'][category[12:]][strval]
                
            elif category in ['gameNames','coopLevelNames']:
                try: translated = self.get_resource('explodinary')['gameNames'][strval]
                except: translated = self.get_resource('translations')[category][strval]
               
            else:
                try: translated = self.get_resource('explodinary')[category][strval]
                except: translated = self.get_resource('translations')[category][strval]
