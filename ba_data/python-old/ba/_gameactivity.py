...
if TYPE_CHECKING:
    ...
    from bastd.actor.bomb import JumpPadSpawner
    from bastd.actor.bomb import SlingshotSpawner
    import ba
...
class GameActivity(Activity[PlayerType, TeamType]):
    ...
    def __init__(self, settings: dict):
        ...
        self._jump_spawners: dict[int, JumpPadSpawner] | None = None
        self._slingshot_spawners: dict[int, SlingshotSpawner] | None = None
        self._tnt_drop_timer: ba.Timer | None = None
        self._jump_drop_timer: ba.Timer | None = None
        self._slingshot_drop_timer: ba.Timer | None = None
    ...
    def _show_info(self) -> None:
        ...
        from explodinary.custom import announcer
        ...
        _ba.timer(0.2, Call(announcer._do_game_announce, _ba.getactivity()))
    ...
    def spawn_player_spaz(
        ...
        # Add the material that allows us to collide with the player-walls.
    ...
    def setup_standard_powerup_drops(self, enable_tnt: bool = True, enable_jump: bool = True, enable_slingshot: bool = True) -> None:
        ...
        if enable_jump:
            self._jump_spawners = {}
            self._setup_standard_jump_drops()
        if enable_slingshot:
            self._slingshot_spawners = {}
            self._setup_standard_slingshot_drops()

    def _standard_drop_powerup(self, index: int, expire: bool = True, pwtype: str | None = None) -> None:
        # pylint: disable=cyclic-import
        from bastd.actor.powerupbox import PowerupBox, PowerupBoxFactory

        pwtype = PowerupBoxFactory.get().get_random_powerup_type(
            forcetype=pwtype, excludetypes=self._excluded_powerups
        )
        if not pwtype: return
    ...
    def _setup_standard_jump_drops(self) -> None:
        """Standard jumppad drop."""
        # pylint: disable=cyclic-import
        from bastd.actor.bomb import JumpPadSpawner

        for i, point in enumerate(self.map.jump_points):
            assert self._jump_spawners is not None
            if self._jump_spawners.get(i) is None:
                self._jump_spawners[i] = JumpPadSpawner(point)
    
    def _setup_standard_slingshot_drops(self) -> None:
        """Standard slingshot drop."""
        # pylint: disable=cyclic-import
        from bastd.actor.bomb import SlingshotSpawner

        for i, point in enumerate(self.map.slingshot_points):
            assert self._slingshot_spawners is not None
            if self._slingshot_spawners.get(i) is None:
                self._slingshot_spawners[i] = SlingshotSpawner(point)
    ...
    def _start_chaos_mode(self,
                          force: bool = False) -> None:
        import ba
        c = ba.app.config
        from explodinary.chaos import GameBlacklist, InitChaosMode
        from explodinary.lib import bseconfig
        
        if (bseconfig.chaos_get('Enabled') and not type(self) in GameBlacklist) or force:
            InitChaosMode(self)
   
        #if c.get("BSE: Speedrun Mode", False):
        #    from ba._speedrun import SpeedrunMode
        #    SpeedrunMode(self) 