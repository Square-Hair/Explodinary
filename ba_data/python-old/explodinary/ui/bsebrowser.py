...
import ba.internal
import random
from bastd.ui.coop import tips
...
class BSECoopBrowserWindow(ba.Window):
    ...
    def __init__(
        ...
        ba.set_analytics_screen('BSE Coop Window')
        ...
        self._plot_button: ba.Widget | None = None
        ...
        if app.ui.use_toolbars and uiscale is ba.UIScale.SMALL:
            ...
            self._back_button = ba.buttonwidget(
                ...
                position=(
                    ...
                    self._height
                    - 87
                    - (49 if uiscale is ba.UIScale.SMALL else 0),
                ),
                size=(120, 60),
                scale=1.2,
                autoselect=True,
        ...
        txt = ba.textwidget(
            ...
            position=(
                self._width * 0.5,
                v + 40 - (49 if uiscale is ba.UIScale.SMALL else 0),
            ),
            size=(0, 0),
            text=ba.Lstr(
        ...
        self.point_tex = ba.gettexture('point')
        ...
        self._scroll_height = self._height - (
            261 if uiscale is ba.UIScale.SMALL else 160
        )
    ...
    def _refresh_campaign_row(self) -> None:
        ...
        self._plot_button = ba.buttonwidget(
            parent=parent_widget,
            label=ba.Lstr(resource='explodinary.plotButtonText'),
            size=(120, 70),
            text_scale=1,
            position=(h + 30, v2 + 80),
            button_type='square',
        ...
        #self._speedrun_button = ba.buttonwidget(
        #    parent=w_parent,
        #    label=ba.Lstr(resource='explodinary.speedrunModeText'),
        #    size=(210, 35),
        #    text_scale=0.85,
        #    position=(730, 560),
        #    color=(0.9, 0.8, 0.25),
    ...
    def _on_plot_info_press(self) -> None:
        ...
        txt = ba.Lstr(resource='explodinary.plotText')
        ConfirmWindow(
            txt,
            text_scale=2,
            cancel_button=False,
    ...
    def _refresh(self) -> None:
        ...
        self._subcontainerheight = (
            620 + (90 if uiscale is ba.UIScale.SMALL else 0)
        )
        ...
        def pick_tip() -> dict:
            ...
            if tip[-4:] == ':WIN': #:WIN windows check
                if not ba.app.platform == 'windows': tip = pick_tip()
                else: tip = tip[:-4]

            return {
                'prefix': ba.Lstr(resource=f'explodinary.bsebrowser.{prefix}.header'),
                'text': ba.Lstr(translate=(f'bsetiptrans.{prefix}', tip[4:])),
                'chaos': is_chaos,
            }
        ...
        tipoff = (90 if uiscale is ba.UIScale.SMALL else 0)
        ba.textwidget(
            parent=w_parent,
        ...
        self._chaos_mode_button = ba.buttonwidget(
            parent=w_parent,
            position=(730, v - 230),
            size=(210, 35),
            on_activate_call=ba.WeakCall(self._chaos_press),
            autoselect=True,
            textcolor=(1, 0.75, 1),
            color=(1.2, 0.4, 1.1),
            label=ba.Lstr(resource='explodinary.chaosModeText'),
        )
        ...
        for item in items:
            ...
            self._custom_buttons.append(
                GameButton(self, sc2, item, h, v2, is_last_sel, 'custom').get_button()
            )
            h += h_spacing
        ...
        for btn in self._custom_buttons:
            ba.widget(
                edit=btn,
                up_widget=self._chaos_mode_button
            )
