...
class AdvancedSettingsWindow(ba.Window):
    ...
    def __init__(
        ...
        if self._do_net_test_button:
            self._sub_height += self._extra_button_spacing
        self._sub_height += self._spacing * 1.0  # plugins
    ...
    def _rebuild(self) -> None:
        ...
        # BSE Supported Langs go here!
        langs_supported: list = [
            'English',
            'Spanish',
            'Polish',
        ]
        
        unsupported_langs: list = languages.copy()
        for x in langs_supported:
            try:
                unsupported_langs.remove(x)
            except: pass

        self._language_popup = LangPopup(
            parent=self._subcontainer,
            position=(210, v - 19),
            width=150,
            ...
            choices_disabled=unsupported_langs,
            button_size=(250, 60),
            choices_display=(
                [
                    ba.Lstr(
        ...
        ba.textwidget(
            ...
            size=(0, 0),
            text=ba.Lstr(
                resource='explodinary.fallbackLangText',
...
class LangPopup(popup_ui.PopupMenu):
    """ A custom popup menu for our languages where we replace disabled choices with red selectable ones. 
        The only function of this class is to redirect to LangPopupWin; the class that builds the window. """
    def __init__(        
        self,
        parent: ba.Widget,
        ...
        super().__init__(parent, position, choices, current_choice, on_value_change_call, opening_call, closing_call, width, maxwidth, scale, choices_disabled, choices_display, button_size, autoselect)
    ...
    def _make_popup(self) -> None:
        ...
        if self._opening_call:
            self._opening_call()
        self._window_widget = LangPopupWin(
...
class LangPopupWin(popup_ui.PopupMenuWindow):
    def __init__(self,
                 position: tuple[float, float],
                 choices: Sequence[str],
                 current_choice: str,
