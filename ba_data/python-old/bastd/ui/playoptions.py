...
class PlayOptionsWindow(popup.PopupWindow):
    ...
    def __init__(
        ...
        # Compensate for our chaos button.
        self._height += 50.0
        ...
        # with checkboxes.
        ccc = self._custom_colors_names_button if self._custom_colors_names_button is not None else self._chaos_mode_button
        for btn in bottom_row_buttons:
            ba.widget(
                edit=btn, down_widget=ccc
            )
        if show_shuffle_check_box:
            ba.widget(
                edit=self._chaos_mode_button,
