...
from explodinary import loader, splashes
...
class MainMenuWindow(ba.Window):
    ...
    def _preload_modules() -> None:
        ...
        import explodinary.ui.settings.allsettings as _unused9
    ...
    def _refresh(self) -> None:
        ...
        if self._have_settings_button and (ba.internal.get_foreground_host_session() is None or
                                           self._in_game or
                                           ba.internal.is_in_replay()):
        ...
        self._editor_notice_text: ba.textwidget | None = None

        from explodinary.game.hub import HubGame
        import _ba
        if False and isinstance(_ba.get_foreground_host_activity(), HubGame):
            self._editor_notice_text =  ba.textwidget(
                parent=self._root_widget,
                position=(h - self._button_width / 2, v - 75),
                size=(self._button_width, self._button_height),
                color=(1, 0.9, 0.4, 1.6),
                scale=0.7,
                h_align='center',
                text=ba.Lstr(resource='explodinary.bseHubCheatNotice',
                             subs=[
                                 ('${KEY}', ' (F5)' if ba.app.platform == "windows" else "")
                             ]),
            )
    ...
    def _refresh_not_in_game(
        ...
        b_count = 4  # play, help, changelog, credits
        ...
        if not self._in_game and enable_account_button:
            self._account_button = btn = ba.buttonwidget(
                color=self.sok_color2,
                parent=self._root_widget,
            ...
            ba.textwidget(
                parent=self._root_widget,
                position=(this_h + 35 - this_b_width - 65, v + 18),
                size=(0, 0),
                scale=0.75,
                transition_delay=self._tdelay,
            ...
            ba.imagewidget(
                ...
                texture=ba.gettexture('accountIcon'),
            )
        # Scattered eggs on easter.
            if (
                ba.internal.get_v1_account_misc_read_val('easter', False)
        ...
        ba.imagewidget(
            ...
            texture=ba.gettexture('gatherIcon'),
        )
        ...
        #SETTINGS
        if self._have_settings_button:
            self._settings_button = btn = ba.buttonwidget(
                color=self.sok_color2,
                parent=self._root_widget,
            ...
            ba.textwidget(
                ...
                draw_controller=btn,
                maxwidth=self._button_width * 0.33,
                text=ba.Lstr(resource=self._r + '.settingsText'),
                h_align='center',
                v_align='center',
            )
        ...
        btn = ba.buttonwidget(
            ...
            autoselect=self._use_autoselect,
            size=(self._button_width, self._button_height),
            label=ba.Lstr(resource='explodinary.changelogText'),
            transition_delay=self._tdelay,
            on_activate_call=self._what_you_know_about_changing_down_in_the_logs,
        )
        self._bse_changelog_button = btn
        ...
        self._credits_button = ba.buttonwidget(
            ...
            scale=scale,
            transition_delay=self._tdelay,
            on_activate_call=self._credits_menu,
        )
        self._tdelay += self._t_delay_inc
    ...
    def _what_you_know_about_changing_down_in_the_logs(self) -> None:
        # pylint: disable=cyclic-import
        from explodinary.ui.changelog import ChangelogWindow
    ...
    def _save_state(self) -> None:
        ...
        elif sel == self._bse_changelog_button:
            ba.app.ui.main_menu_selection = 'BSEChangelog'
    ...
    def _restore_state(self) -> None:
        ...
        if sel_name == 'BSEChangelog':
            sel = self._bse_changelog_button
