...
class GameButton:
    ...
    def __init__(
        ...
        vd = _versiondata.levelsub
        bsespecial = (
            f'{vd}:Pathway Pandemonium',
            f'{vd}:Endless Kablooya Onslaught',
            f'{vd}:Route Roulette'
        )
        
        if game in bsespecial:
            self._players_widget = ba.textwidget(
                parent=parent,
                draw_controller=btn,
                position=(x + 20 + sclx * 0.5, y + scly - 165),
                size=(0, 0),
                h_align='center',
                text='1-5 players',
                v_align='bottom',
        ...
        xscl = x + (67 if self._achievements or game in bsespecial else 50)
        
        yscl = y + scly - (137 if self._achievements or game in bsespecial else 157)

        starscale = 35.0 if self._achievements or game in bsespecial else 45.0
    ...
    def _update(self) -> None:
        ...
        # is selected.
        unlocked_color = (
            (0.1, 0.85, 0.45) if game.startswith('Easy:') else (0.1, 0.6, 0.3)
        )
        ...
        from explodinary import _versiondata
        vd = _versiondata.levelsub
        if game in (f'{vd}:BSE Infinite Runaround',
                    f'{vd}:BSE Infinite Onslaught'):
            ba.buttonwidget(
            edit=self._button,
            color=(0.1, 0.6, 0.3),
            scale=1,
        )
        if game in (f'{vd}:Kablooya Onslaught',
                    f'{vd}:Endless Kablooya Onslaught'):
            ba.buttonwidget(
            edit=self._button,
            color=(0.6, 0.85, 0.3),
        ...
        if game in (f'{vd}:Kablooya Runaround',
                    f'{vd}:Endless Kablooya Runaround'):
            ba.buttonwidget(
            edit=self._button,
            color=(0.4, 0.65, 0.05),
            scale=1,
        )
        if game in (f'{vd}:Pathway Pandemonium'):
            ba.buttonwidget(
            edit=self._button,
            color=(0.3, 0.5, 0.8),
            scale=1,
        ...
        #if game in (f'{vd}:Route Roulette'):
        #    ba.buttonwidget(
        #    edit=self._button,
        #    color=(0.65, 1, 0.2),
        #    scale=1,
