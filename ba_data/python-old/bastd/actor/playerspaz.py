...
class PlayerSpazVitalMessage:
    """ A message saying a PlayerSpaz got vitalized. """
    spaz: PlayerSpaz

    def __init__(self, spaz: PlayerSpaz):
        self.spaz = spaz

class PlayerSpazVitaminMessage:
    """ A message saying a PlayerSpaz picked up a vitamin. """
    spaz: PlayerSpaz
...
class PlayerSpaz(Spaz):
    ...
    def handlemessage(self, msg: Any) -> Any:
        ...
        # Keep track of the player who last hit us for point rewarding.
            ...
            activity = self._activity()
            if activity is not None and self._player.exists():
                activity.handlemessage(PlayerSpazVitalMessage(self))

        elif isinstance(msg, ba.VitaminMessage):
            super().handlemessage(msg)  # Augment standard behavior.
            activity = self._activity()
            if activity is not None and self._player.exists():
                activity.handlemessage(PlayerSpazVitaminMessage(self))
