...
class RaceGame(ba.TeamGameActivity[Player, Team]):
    ...
    def get_available_settings(
        ...
        settings = [
            ...
            ba.BoolSetting('Enable Mine Variants', default=False),
            
            ba.BoolSetting('Equip Dash on Spawn', default=False),
    ...
    def __init__(self, settings: dict):
        ...
        self._enable_mine_variants = bool(settings['Enable Mine Variants'])
        self._dash_enabled = bool(settings['Equip Dash on Spawn'])
    ...
    def _make_mine(self, i: int) -> None:
        ...
        if self._enable_mine_variants:
            mines = ('land_mine', 'lite_mine', 'flutter_mine')
        else:
            mines = ('land_mine', 'land_mine')
        rmine = self._race_mines[i]
        rmine.mine = Bomb(position=rmine.point[:3], bomb_type=random.choice(mines))
        rmine.mine.arm()
    ...
    def spawn_player(self, player: Player) -> ba.Actor:
        ...
        if self._dash_enabled:
            spaz.node.handlemessage(ba.PowerupMessage('dash'))
